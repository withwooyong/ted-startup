...
| 항목 | 해결 |
|---|---|
| R-01 HIGH: DIP 미완성 (application 이 adapter.out.external 직접 import) | `app/application/dto/kis.py` + `app/application/port/out/kis_port.py` 신설. `portfolio_service.py` 의 adapter import 완전 제거. `KisHoldingsFetcher` Protocol(structural typing), port 예외 `KisUpstreamError` + `KisCredentialRejectedError` 로 수렴 |
| R-02 HIGH: Router account 이중 로드 | `_ensure_kis_real_account` + 3 UseCase `execute()` 에 optional `account` 파라미터 추가. Router 가 선로드한 account 를 명시 전달 |
| 리뷰 HIGH(본 PR 에서 발생): `KisNotConfiguredError(KisUpstreamError)` | `KisNotConfiguredError(Exception)` 으로 분리 — 서버 설정 오류를 UseCase `except KisUpstreamError` 가 삼켜 502 로 오진단하는 경로 차단 |
| 리뷰 MEDIUM: `test_connection` docstring `KisAuthError` 잔존 | port 이름으로 현행화 |
...
### PR #24 — refactor: KisAuthError 4xx/5xx 분리 (`77903d9`)

PR 5 (#16) 리뷰 이월 MEDIUM 해소. HTTP 401/403 → `KisCredentialRejectedError` → `CredentialRejectedError` → HTTP 400, 5xx/네트워크 → `SyncError` → HTTP 502. 예외 메시지의 `body=` 제거 후 DEBUG 로그로 분리. 테스트 8건 추가 (+8). **CI 5/5 PASS**.
...
### /review 세션 감사 (PR #22·#23·#24 통합)

3 PR 머지 후 /review 로 세션 누적 코드 리뷰 + 보안 감사. 리포트 2개 산출:
- `pipeline/artifacts/08-review-report/review-2026-04-22-session.md` (CRITICAL 0, HIGH 2, MEDIUM 3, LOW 1)
- `pipeline/artifacts/09-security-audit/audit-2026-04-22-session.md` (CRITICAL 0, HIGH 0, MEDIUM 1, LOW 2, INFO 2)

HIGH 2건 (R-01 DIP 미완성 + R-02 Router 이중 로드) 이 본 PR 로 통합 해소. 나머지 MEDIUM/LOW 는 백로그.

### 차기 후보 (2026-04-22 세션 마감 후)

1. ~~#2 CI lint/type 게이트~~ ✅ PR #22
2. ~~#3 Hexagonal + #4 mock/real 분리~~ ✅ PR #23
3. ~~#5 KisAuthError 401/5xx 분리~~ ✅ PR #24
4. ~~R-01+R-02 DIP 완성 + account 단일 로드~~ 🔄 이 PR (커밋 대기)
5. **다른 서비스(Telegram/Krx/Dart) DIP 확장** (1~2h) — 본 PR 의 leading example 확장
6. **#1 모바일 반응형 착수** (3.5~4 man-day) — 최대 사용자 가치
7. **DB 모델 Mapped[str] → Literal 좁히기** (PR #23 리뷰 파생) — Router exhaustive check 가능
8. **R-04 MOCK 401 경로 재분류** (/review 세션 감사) — MOCK 운영자 오류를 사용자 응답 400 이 아닌 500/502 로
9. **R-05 /sync endpoint 400 응답 테스트** (/review 세션 감사) — `test_endpoint_sync_real_account_credential_rejected_400` 추가
10. **R-06 `.git-blame-ignore-revs`** — PR #22 format 커밋 투명화
11. **Exception 이름 중복 완화** — R-03 follow-up
12. **#6 asyncio_mode=auto 마이그레이션** (소규모)
13. **#7 KIS sync 회고 문서** (선택적)
