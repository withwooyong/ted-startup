...
> Last updated: 2026-04-17 (세션 종료, v1.0 배포 준비 완료)
> Branch: `master`
> Latest commit: `764d6d3` — Phase 5 Ship 완료: 배포 인프라 + KPI + Judge PASS 8.1/10
> **Uncommitted**: 5 files (H-1 env 정합 수정 + `proxy.ts` 신규) — /ted-run Step 4에서 대기 중

---

## Current Status

**v1.0 배포 직전 상태.** Phase 1~5 전체 파이프라인 통과 완료 (Discovery 8.1 · Design 8.3 · Build sprint 1-4 · Verify 7.6 · Ship 8.1). 5개 파일이 uncommitted — 코드 리뷰에서 발견한 **HIGH H-1(프론트/백엔드 env 변수명 불일치)** 블로커를 /ted-run 파이프라인으로 수정했고, 빌드/리뷰 재검증까지 통과했으나 커밋은 사용자 확인 대기 중.
...
| 4 | 코드 리뷰 재검증에서 HIGH H-1(env 변수명 불일치) 발견 → /ted-run으로 수정 | *uncommitted* | proxy.ts (신규), next.config.ts, route.ts, client.ts, backend Dockerfile |

## In Progress / Pending

| # | Task | Status | Notes |
|---|------|--------|-------|
| 1 | /ted-run Step 4: H-1 fix 커밋 + 푸시 | 사용자 확인 대기 | 변경 5 files 검토 후 commit/push 결정 필요 |
| 2 | Ship+48h: Reverse proxy + TLS (Caddy/Nginx + Let's Encrypt) 구축 | P0 권고 | Judge Ship 리포트에서 필수 격상 권고. `ADMIN_API_KEY` 평문 전송 회피 |
| 3 | Ship+7d: Spring Actuator `management.endpoints.web.exposure.include=health,metrics,prometheus` 활성화 | P0 권고 | Week 1 스테빌리티 베이스라인 수집. 1줄 수정 |
| 4 | Flyway 도입 (V3+ 자동 마이그레이션) | v1.1 | 현재 V1/V2 SQL은 compose initdb로 최초 부팅 시 1회만 실행 |
| 5 | 프론트엔드 테스트 하네스 (Vitest + RTL + MSW) | v1.1 | QA 리포트 Top 권고 — 회귀 보호 부재 |
| 6 | TanStack Query 도입 (4개 페이지 리팩터) | v1.1 / v2 | `useEffect + fetch` 패턴 일소, F-M1 |
| 7 | 한국 공휴일 캘린더 연동 | v1.1 | `MarketDataPersistService.findPreviousTradingDate` TODO |
...
- **rewrites 대신 proxy.ts** — Next.js `next.config.ts` rewrites는 build time에 routes-manifest.json으로 고정되어 런타임 env 반영 불가. Next.js 16 canonical `proxy.ts` 파일 컨벤션 사용(구 middleware 대체).
...
- ✅ HIGH H-1 (uncommitted): env 변수명 3중 불일치 — compose/client/route.ts 모두 `NEXT_PUBLIC_API_BASE_URL` + `BACKEND_INTERNAL_URL` 단일화
...
- 사용자 커밋 철학: 한글 메시지 + 푸시는 명시 요청 시에만. 커밋과 푸시를 분리.
...
### 다음 세션 첫 액션 권고
1. **uncommitted 5 files 검토 후 커밋** — 커밋 메시지 초안은 `CHANGELOG.md [Unreleased]` 섹션에 이미 정리됨. H-1 fix + M-1 + M-4 병합 단일 커밋 권장.
2. **(선택) 푸시** — 사용자 명시 요청 시.
3. **실제 VPS에서 첫 배포 스모크 테스트** — `cp .env.prod.example .env.prod` → 실제 값 입력 → `docker compose --env-file .env.prod -f docker-compose.prod.yml up -d --build`. runbook.md §5 스모크 테스트 섹션 참조.
4. **Ship+48h P0 작업**: Reverse proxy + TLS 구축. `runbook.md §1` 선택 → 필수 격상.

### 제약/선호
- **Claude 모델**: Opus 4.7 (1M context) 단일 운영. Max 구독자.
- **Git 규칙**: 한글 커밋 메시지 / 커밋·푸시 분리 / `git push` 명시 요청 전 실행 금지
- **Next.js**: v16.2.4 — training data와 다름. `node_modules/next/dist/docs/`로 확인 후 작업 (AGENTS.md 지시).
- **프론트엔드 상태관리 컨벤션**: TanStack Query (서버) + Zustand (클라이언트) — 현재 미준수, v1.1 부채.

## Files Modified This Session

### 커밋된 변경 (3 commits, 26 files, +2,488 / -190)
```
871ff57  프로토타입 효과 3종 Next.js 이식 (6 files, +271/-30)
eb5fc15  Phase 4 Verify + 블로커 4건 수정 (10 files, +1013/-158)
764d6d3  Phase 5 Ship 완료 (11 files, +1210/-8)
```

### Uncommitted (H-1 fix, 5 files)
```
M  src/backend/Dockerfile                                                     (M-1: || true 제거)
M  src/frontend/next.config.ts                                                (rewrites 제거 + 주석)
M  src/frontend/src/app/api/admin/notifications/preferences/route.ts          (BACKEND_INTERNAL_URL, /api prefix, 16KB guard)
M  src/frontend/src/lib/api/client.ts                                         (NEXT_PUBLIC_API_BASE_URL)
?? src/frontend/src/proxy.ts                                                  (신규: 런타임 /api/* → backend 프록시)
```

### 빌드 검증 상태
- `next build`: 6 pages + Proxy + Route Handler 모두 성공
- `./gradlew test`: 29/29 통과 (UP-TO-DATE)
