"""IngestShortSellingUseCase 단건 sentinel catch 회귀 (Phase F-3 D-7).
...
D-7 설계 근거:
- 단건 UseCase 는 라우터가 없는 경로에서도 직접 호출될 수 있음 (CLI / bulk / 테스트)
...
- bulk loop 의 sentinel 분리와 일관성 유지 (F-2 결정 #3)
...
from app.application.service.short_selling_service import IngestShortSellingUseCase

# ---------------------------------------------------------------------------
# Fixtures — test_short_selling_service_sentinel.py 패턴 1:1 차용
# ---------------------------------------------------------------------------
...
def _make_sentinel_raising_client(stock_code: str) -> KiwoomShortSellingClient:
    """fetch_trend 호출 시 SentinelStockCodeError 를 raise 하는 stub client."""
    ...
    async def _fetch_trend(
        ...
        *,
        ...
        raise SentinelStockCodeError(sc)

    client.fetch_trend = _fetch_trend
    return client


def _make_kiwoom_error_client() -> KiwoomShortSellingClient:
    """fetch_trend 호출 시 KiwoomBusinessError 를 raise 하는 stub client."""
    client = AsyncMock(spec=KiwoomShortSellingClient)
...
def _make_normal_client() -> KiwoomShortSellingClient:
    """fetch_trend 호출 시 빈 list 를 반환하는 정상 stub client."""
    client = AsyncMock(spec=KiwoomShortSellingClient)
...
async def test_execute_sentinel_catch_returns_outcome_with_skip_reason(
    ...
    await _create_active_stock(session, "000A1B", "알파넘릭테스트")

    client = _make_sentinel_raising_client("000A1B")
...
async def test_execute_normal_path_unchanged(
    ...
    fetch_trend → 빈 list → upserted=0, error=None, skipped=False.
    """
...
async def test_execute_kiwoom_error_still_propagates_via_outcome(
    ...
    """KiwoomBusinessError (네트워크/비즈니스) 는 기존 동작 유지 — outcome.error = 'business:{return_code}'.
