"""Phase F-2 — slb.py 알파벳 포함 종목코드 가드 TDD (Step 0, red).

KiwoomLendingClient.fetch_stock_trend 가 알파벳 포함 종목코드 (00088K / TIGER1 / 005930_NX 등)
에 대해 SentinelStockCodeError 를 raise 하는지 검증.

본 테스트는 구현 전 의도적으로 실패 (red) — Step 1 구현 후 green 전환 대상.

계획서 § 4 확정 결정:
- #1 slb.py 가드 raise type → SentinelStockCodeError (F-1 신설 type 재사용)
...
# ---------------------------------------------------------------------------
# 가드 — slb.py 는 현재 ValueError raise → SentinelStockCodeError raise 필요 (Step 1 fix)
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "005930_NX", # NXT suffix — 8자리, `_NX` 포함 (plan § 12.2 #4 명시 거부 대상)
    ],
)
def test_fetch_stock_trend_raises_sentinel_error_for_alphanumeric_code(stock_code: str) -> None:
    """alphanumeric / NXT suffix 종목코드 → SentinelStockCodeError raise.

    계획서 § 4 #1: slb.py 가드 raise type = SentinelStockCodeError.
    현재 구현은 ValueError raise → SentinelStockCodeError 로 변경 필요 (Step 1 fix = red).

    주의:
    - 6자리 숫자 (^[0-9]{6}$) 만 통과.
    - `005930_NX` 는 8자리이며 `_` / `N` / `X` 포함 → 거부.
    - plan § 12.2 #4: ka20068 는 KRX only, NXT suffix 명시 거부.
    """
    ...
    client = KiwoomLendingClient(kiwoom_client=mock_client)

    with pytest.raises(SentinelStockCodeError):
        import asyncio

        asyncio.run(
            client.fetch_stock_trend(
...
def test_fetch_stock_trend_sentinel_error_is_catchable_as_value_error() -> None:
    ...
    assert caught_as_sentinel, (
        ...
        "slb.py 가드 raise type 을 SentinelStockCodeError 로 변경 필요 (Step 1)"
    )


def test_fetch_stock_trend_sentinel_error_message_contains_stock_code() -> None:
...
def test_fetch_stock_trend_sentinel_error_message_contains_nx_suffix() -> None:
    """NXT suffix 종목코드 (005930_NX) — 메시지에 종목코드 포함.

    plan § 12.2 #4: _NX suffix 명시 거부 가시성 확보.
    """
    from app.adapter.out.kiwoom.slb import KiwoomLendingClient
    from app.adapter.out.kiwoom.stkinfo import SentinelStockCodeError
    ...
    with pytest.raises(SentinelStockCodeError) as exc_info:
        ...
        asyncio.run(
            client.fetch_stock_trend(
                stock_code="005930_NX",
                start_date=date(2025, 1, 1),
                end_date=date(2025, 1, 31),
            )
        )

    assert "005930_NX" in str(exc_info.value), (
        "SentinelStockCodeError 메시지에 NXT suffix 종목코드 포함 필요"
    )
...
def test_fetch_stock_trend_does_not_raise_for_normal_numeric_code() -> None:
