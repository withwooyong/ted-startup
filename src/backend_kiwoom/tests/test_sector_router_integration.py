"""POST /api/kiwoom/sectors/sync 풀 체인 통합 — 라우터 → 실 UseCase → MockTransport → DB.

#4 sector_service 단위 테스트가 UseCase 풀 체인을 다루고, #5 sector_router 단위 테스트가
admin guard / DTO / F3 hint 를 다룸. 본 모듈은 그 둘을 묶은 회귀 1 케이스 — main.py
lifespan 의 factory 패턴이 라우터에서 정상 동작하는지.

KiwoomCredential row 생성 + ASGI 라우터 → 실 SyncSectorMasterUseCase → MockTransport
→ testcontainers PG16 → sector 테이블 적재 검증.
"""
...
async def cleanup_db(
    ...
    """sector + credential 테이블 매 테스트마다 비움."""
...
async def test_router_post_sync_full_chain_writes_to_db(
    ...
    """라우터 POST /sectors/sync → 실 UseCase factory → MockTransport → DB 적재."""
    ...
    # 2. MockTransport — au10001 (토큰 발급) + ka10101 (5 시장 응답)
    ...
    # 3. TokenManager — sessionmaker 가 commit 모드
    @asynccontextmanager
    ...
    # 4. SyncSectorUseCaseFactory — main.py lifespan 패턴 그대로
    @asynccontextmanager
    async def _sync_factory(
        alias: str,
    ) -> AsyncIterator[SyncSectorMasterUseCase]:
    ...
    app.dependency_overrides[get_sync_sector_factory] = lambda: _sync_factory

    # 6. 호출
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://testserver") as client:
        resp = await client.post(
            "/api/kiwoom/sectors/sync",
            params={"alias": "test-integration"},
    ...
    # 8. DB 검증 — 실제로 sector 테이블 적재됨
    async with commit_sessionmaker() as s:
        repo_check = SectorRepository(s)
        all_rows = await repo_check.list_all(only_active=True)

    assert len(all_rows) == 5
    assert {r.market_code for r in all_rows} == {"0", "1", "2", "4", "7"}
