...
설계: endpoint-10-ka10086.md § 7.1 + C-1β ohlcv router 패턴 일관.

검증:
- POST /daily-flow/sync — admin, base_date + only_market_codes 옵션
- POST /stocks/{code}/daily-flow/refresh — admin, KiwoomError 매핑, ValueError → 404
- GET /stocks/{code}/daily-flow?exchange=&start=&end= — DB only
"""
...
from app.adapter.web._deps import get_ingest_daily_flow_factory
...
def _clear_global_engine_cache() -> Iterator[None]:
    ...
    C-1β test_ohlcv_router.py 패턴 일관 — 다른 파일에서 캐시된 engine 을 받아 다른
    event loop 에 바인딩될 때 RuntimeError 차단.
    """
...
def _stub_factory(uc: IngestDailyFlowUseCase) -> Any:
    @asynccontextmanager
...
def _make_app(factory: Any = None) -> FastAPI:
    ...
    if factory is not None:
        app.dependency_overrides[get_ingest_daily_flow_factory] = lambda: factory
    return app
...
async def test_sync_returns_401_without_admin(admin_key: str) -> None:
    ...
    async with _client(app) as cl:
        resp = await cl.post("/api/kiwoom/daily-flow/sync?alias=test")
    assert resp.status_code == 401


@pytest.mark.asyncio
async def test_sync_returns_result_for_admin(admin_key: str) -> None:
    uc = AsyncMock(spec=IngestDailyFlowUseCase)
    uc.execute = AsyncMock(
        return_value=DailyFlowSyncResult(
    ...
    async with _client(app) as cl:
        resp = await cl.post(
            "/api/kiwoom/daily-flow/sync?alias=test",
...
async def test_sync_passes_optional_body(admin_key: str) -> None:
    ...
    async def _execute(
        *, base_date: date | None = None, only_market_codes: list[str] | None = None
    ) -> DailyFlowSyncResult:
...
# =============================================================================
# POST /stocks/{code}/daily-flow/refresh — admin
# =============================================================================
...
async def test_refresh_returns_401_without_admin(admin_key: str) -> None:
    app = _make_app()
    async with _client(app) as cl:
        resp = await cl.post("/api/kiwoom/stocks/005930/daily-flow/refresh?alias=test")
    assert resp.status_code == 401
...
async def test_refresh_returns_result_for_admin(admin_key: str) -> None:
    ...
    async with _client(app) as cl:
        resp = await cl.post(
            "/api/kiwoom/stocks/005930/daily-flow/refresh?alias=test",
            headers={"X-API-Key": admin_key},
        )
...
async def test_refresh_maps_business_error_to_400(admin_key: str) -> None:
    ...
    uc = AsyncMock(spec=IngestDailyFlowUseCase)
    uc.refresh_one = AsyncMock(
        side_effect=KiwoomBusinessError(api_id="ka10086", return_code=1, message="존재하지 않음")
    )
    app = _make_app(_stub_factory(uc))

    async with _client(app) as cl:
        resp = await cl.post(
            "/api/kiwoom/stocks/999999/daily-flow/refresh?alias=test",
            headers={"X-API-Key": admin_key},
        )
...
@pytest.mark.asyncio
async def test_get_daily_flow_three_scenarios_in_single_lifetime(session: AsyncSession) -> None:
    """Stock 미존재 / 빈 시계열 / 데이터 있음 — 한 client lifetime 안 (B/C 패턴 일관)."""

    app = _make_app()
    async with _client(app) as cl:
        # 1. Stock 미존재
        resp1 = await cl.get(
            "/api/kiwoom/stocks/005930/daily-flow?start=2025-09-01&end=2025-09-30"
        )
        ...
        # C-2δ — C/E 중복 2 필드도 응답 DTO 에서 DROP
        assert "credit_balance_rate" not in body[0]
        assert "foreign_weight" not in body[0]


@pytest.mark.asyncio
async def test_get_daily_flow_rejects_inverted_window() -> None:
    """start > end → 400."""
    app = _make_app()
    async with _client(app) as cl:
        resp = await cl.get(
            "/api/kiwoom/stocks/005930/daily-flow?start=2025-09-30&end=2025-09-01"
        )
    assert resp.status_code == 400
...
@pytest.mark.asyncio
async def test_get_daily_flow_rejects_oversized_range() -> None:
    """date range > 400일 → 400 (DoS amplification 차단, C-1β 2b-M1 패턴 일관)."""
    app = _make_app()
    async with _client(app) as cl:
        resp = await cl.get(
            "/api/kiwoom/stocks/005930/daily-flow?start=1900-01-01&end=2099-12-31"
        )
    assert resp.status_code == 400
...
@pytest.mark.asyncio
async def test_get_daily_flow_rejects_sor_exchange() -> None:
    """SOR 미지원 — Phase D 결정 (Repository 가 ValueError → 400 매핑)."""
    app = _make_app()
    async with _client(app) as cl:
        resp = await cl.get(
            "/api/kiwoom/stocks/005930/daily-flow?start=2025-09-01&end=2025-09-30&exchange=SOR"
        )
    assert resp.status_code == 422  # pattern 검증 단계에서 차단
