"""일간 daily flow sync 콜백 — APScheduler 가 트리거 (C-2β).

설계: endpoint-10-ka10086.md § 7 + ADR § 18 (C-1β 패턴 일관 — cron 19:00 KST mon-fri).

책임 (ohlcv_daily_job 패턴 일관):
- `IngestDailyFlowUseCaseFactory` 호출 → `IngestDailyFlowUseCase.execute`
...
호출자:
- `DailyFlowScheduler` 의 등록된 daily cron job (KST 19:00)
...
async def fire_daily_flow_sync(
    *,
    ...
    """
    logger.info("daily flow sync cron 시작 — alias=%s", alias)
    try:
