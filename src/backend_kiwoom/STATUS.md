...
- **Phase D — scheduler misfire_grace_time 통일** (5-14, ted-run) — § 42.5 옵션 C + 보조 E 채택 (노트북 + 학습 우선). plan doc 신규 (`phase-d-scheduler-misfire-grace.md`). 12 스케줄러 클래스 모두 `MISFIRE_GRACE_SECONDS: Final[int] = 21600` (6h) 통일 — 9 신규 (SectorSync / StockMaster / StockFundamental / OhlcvDaily / DailyFlow / WeeklyOhlcv / MonthlyOhlcv / YearlyOhlcv / SectorDailyOhlcv) + 3 갱신 (Short 1800→21600 / LendingMarket 1800→21600 / LendingStock 5400→21600). **2b M-2 fix**: `app/main.py /admin/scheduler/diag` jobs dump 에 `misfire_grace_time` 노출 (운영 가시화). **2a M-1/M-2/M-3 fix**: scheduler.py L94 / L1122 + test_scheduler_phase_e.py docstring 구값 갱신. **2b M-1 plan doc § 5 H-6 보강**: cross-scheduler race (5+ cron 동시 catch-up + KiwoomClient 인스턴스 단위 lock) — 운영 모니터 + 위반 시 별도 chunk. Verification 5관문 PASS (ruff clean / mypy strict 95 files / pytest 1199 / cov **86.17%** / 보안 ⚪ 일반 분류 / 런타임 ✅ collection / E2E ⚪ UI 0). 코드 10 파일 +78/-19. Migration 0. ADR § 43 신규 + § 42.8 갱신. `<this commit>`
