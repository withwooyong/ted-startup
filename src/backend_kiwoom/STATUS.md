...
> **마지막 갱신**: 2026-05-14 (Phase D-1 follow-up ted-run 풀 파이프라인 완료 — MaxPages cap 상향 (ka20006 10→40 / ka10086 40→60) + bulk insert 32767 chunk 분할 (`_chunked_upsert` helper + sector_price / daily_flow Repository 적용) + `KiwoomMaxPagesExceededError(api_id, page, cap)` 시그니처 확장. 1R+2R PASS (CRITICAL/HIGH 0, MEDIUM 3 즉시 fix) / Verification 5관문 PASS / 6 코드 + 5 테스트 (4 갱신 + 1 신규) / **1199 tests** (1186 → +13 신규) / coverage **86.13%** / ruff + mypy strict PASS / ADR § 41 신규. 컨테이너 재배포 + 5-12 운영 백필 재호출은 E4 별도 chunk)
...
| 마지막 완료 chunk | **Phase D-1 follow-up 풀 구현 (ted-run)** — `SECTOR_DAILY_MAX_PAGES = 10 → 40` (chart.py) + `DAILY_MARKET_MAX_PAGES = 40 → 60` (mrkcond.py) + `KiwoomMaxPagesExceededError(*, api_id, page, cap)` 시그니처 확장 + `_chunked_upsert(session, statement_factory, rows, *, chunk_size=1000) -> int` helper 신규 (`_helpers.py`, `n_cols × chunk_size > 32767` fail-fast 가드 포함) + `SectorPriceDailyRepository.upsert_many` chunk 적용 + `StockDailyFlowRepository.upsert_many` chunk 적용. 1R 2a (sonnet) + 2b (opus) PASS / MEDIUM 3 즉시 fix (docstring 정합 + stateless 명시 + col 가드) / Verification 5관문 PASS / 6 파일 + 5 테스트 (4 갱신 + 1 신규 / +13 cases) / **1199 tests** / coverage **86.13%** / ruff + mypy strict PASS / ADR § 41 신규 |
| 다음 chunk | **(E4) 컨테이너 재배포 + 5-12 운영 백필 재호출** — alembic 0 / docker compose build + restart → 12 scheduler 유지 + cap 상향 적용 → sector_daily 64 sector + KOSDAQ ~1814 종목 재호출 → 0 MaxPages / 0 InterfaceError 검증 / ADR § 41.7 운영 결과 채움. **(F chunk)** ka10001 NUMERIC(8,4) overflow + sentinel WARN/skipped 분리 별도 ted-run |
| 25 Endpoint 진행 | **15 / 25 완료 (60%)**. ka10014 / ka10068 / ka20068 → ✅ |
| 누적 chunk | 51+ commits (Phase D-1 follow-up 풀 구현 chunk 추가 예정) |
| 테스트 | **1199 cases** (1186 → +13 신규) / coverage **86.13%** / ruff PASS / mypy strict PASS |
...
| ~~27~~ | ~~ka20006 sector_daily 60% 실패 — KiwoomMaxPagesExceededError + InterfaceError~~ | 본 chunk 코드 fix 완료 | ✅ **코드 fix 완료** (`<this chunk>`) — `SECTOR_DAILY_MAX_PAGES = 10 → 40` + `_chunked_upsert` chunk_size=1000 적용 (sector_price Repository) + `KiwoomMaxPagesExceededError(api_id, page, cap)` 시그니처. 5500 row chunk 분할 testcontainers PG16 PASS. **운영 5-12 재호출은 E4 별도 chunk** |
| ~~28~~ | ~~ka10086 KOSDAQ 1814 누락 — `DAILY_MARKET_MAX_PAGES=40` 부족~~ | 본 chunk 코드 fix 완료 | ✅ **코드 fix 완료** (`<this chunk>`) — `DAILY_MARKET_MAX_PAGES = 40 → 60` + `_chunked_upsert` daily_flow Repository 적용 (3000 row chunk 분할 PG16 PASS). **운영 5-12 KOSDAQ 재호출은 E4 별도 chunk** |
...
| 순위 | chunk | 근거 | 예상 규모 |
|------|-------|------|-----------|
| **1** | **(E4) 컨테이너 재배포 + 5-12 운영 백필 재호출** | 본 chunk 머지 후. docker compose build + restart → cap 상향 + chunk 분할 적용 → sector_daily 64 sector + KOSDAQ ~1814 종목 재호출 → 0 MaxPages / 0 InterfaceError 검증. ADR § 41.7 운영 결과 채움 | 빠름 — 재빌드 + admin endpoint 호출 |
| **2** | **F chunk — ka10001 NUMERIC overflow + sentinel WARN/skipped 분리** | § 4 #29. Migration 신규 (NUMERIC(8,4) precision 확대 — overflow 종목 값 분석 선행) + WARN/skipped 분리 + result.errors 의 full exception type/메시지 로그 보강 | 별도 ted-run (E와 독립) |
...
- **Phase D-1 follow-up 풀 구현** (5-13, ted-run) — `SECTOR_DAILY_MAX_PAGES = 10 → 40` (chart.py) + `DAILY_MARKET_MAX_PAGES = 40 → 60` (mrkcond.py) + `KiwoomMaxPagesExceededError(*, api_id, page, cap)` 시그니처 확장 (`_client.py`, raise site 갱신) + `_chunked_upsert(session, statement_factory, rows, *, chunk_size=1000) -> int` helper 신규 (`_helpers.py`, **2b 2R M-1**: `n_cols × chunk_size > 32767` fail-fast 가드 + **2a 2R M-2**: factory stateless docstring 명시) + `SectorPriceDailyRepository.upsert_many` `_chunked_upsert` 적용 (8 col × 1000 = 8000 args/chunk) + `StockDailyFlowRepository.upsert_many` 동일 패턴 (12 col × 1000 = 12000 args/chunk). 1R 2a (sonnet) PASS + 2a 2R M-1 (chart.py:742 docstring `(10)` → `(40)` 정합) fix. 1R 2b (opus) PASS + 2b 2R M-1 (col 가드) fix. Verification 5관문 PASS (ruff: clean / mypy strict: 95 files Success / pytest: **1199 passed** / coverage **86.13%** / 보안스캔 ⚪ 계약분류 / 런타임 ✅ import collection / E2E ⚪ UI 변경 0). 코드 6 파일 + 테스트 5 (4 갱신 + 1 신규) = +13 cases. Migration 0 / UseCase 변경 0. ADR § 41 신규. `<this commit>`
