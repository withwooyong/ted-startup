...
> **마지막 갱신**: 2026-05-14 (scheduler dead 원인 확정 분석 — pmset -g log 결정적 증거: 5-13 20:01~21:12 Mac 반복 Sleep + 현재 caffeinate 활성 (5-13 비활성). Mac 절전 → Docker VM sleep → APScheduler timer 미발화 → 5-14 06:00/06:30 cron miss. ADR § 42 신규 (원인 + 가설 평가 + 해결 옵션 A~E + § 38.8 #1 갱신). 사용자 환경 결정 대기. 코드 변경 0)
...
| 마지막 완료 chunk | **scheduler dead 원인 확정 분석** — § 41.8 발견 (5-14 06:00/06:30 cron miss) 후속. **pmset -g log 결정적 증거**: 5-13 20:01~21:12 Mac 반복 Sleep + DarkWake 사이클 (Battery 모드, Charge 80%). 현재 `sleep prevented by caffeinate*3` 활성 (5-13 비활성). 가설 평가표 5종 (Mac 절전 ✅ 확정 / race 반증 / network 반증 / healthcheck 반증 / battery 반증). cron 별 misfire 정책 정리 (07:30/07:45/08:00 만 misfire grace, 06:00/06:30/03:00 새벽 cron 위험). ADR § 42 신규 + § 38.8 #1 갱신 (가설 → 확정). 해결 옵션 A~E (caffeinate / 서버 이전 / misfire 전체 적용 / launchd cron / 현재 유지). 코드 변경 0 |
| 다음 chunk | **사용자 환경 결정** (§ 42.5 옵션 A~E) — Mac 절전 차단 (caffeinate 영구 활성) vs 별도 Linux 서버 이전 vs 현재 유지. 결정 후 후행 chunk. 또는 **F chunk** (ka10001 NUMERIC overflow + sentinel WARN/skipped 분리, Mac 결정과 독립) 별도 ted-run |
