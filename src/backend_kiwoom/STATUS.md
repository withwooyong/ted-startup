...
> **마지막 갱신**: 2026-05-14 (Phase D-1 follow-up E4 운영 백필 재호출 완료 — 컨테이너 재배포 + 5-12 sector_daily 124/124 success (이전 60/124) + KOSDAQ daily_flow 1487 success / 0 failed / 7m 7s. 0 MaxPages / 0 InterfaceError 검증. ADR § 41.7 운영 결과 채움 + § 41.8 5-14 새벽 cron miss 추가 발견 (scheduler dead 재발 가능성 — 별도 chunk 후보). 코드 변경 0)
...
| 마지막 완료 chunk | **Phase D-1 follow-up E4 — 컨테이너 재배포 + 5-12 운영 백필 재호출** — docker compose build + up -d → 12 scheduler 활성 (06:51 KST). sector_daily 5-12 bulk sync 호출 (06:58) **124/124 success / 0 failed** (이전 60/124). KOSDAQ daily_flow backfill CLI (06:53~07:00) **1487 success / 0 failed / 7m 7s** (avg 0.3s/stock). 0 MaxPages / 0 InterfaceError 검증 완료. ADR § 41.7 운영 결과 표 채움. **5-14 06:00/06:30 cron miss** 추가 발견 (§ 41.8) — scheduler dead 재발 가능성, 별도 chunk 후보. 코드 변경 0 |
| 다음 chunk | **scheduler dead 재발 분석 chunk** — 5-14 06:00 ohlcv_daily / 06:30 daily_flow cron 발화 0건 (재배포 직전 1시간 docker logs). 5-13 17:30/18:00 정상 → 5-14 06:00 miss = 일회성 가설 흔들림. APScheduler timer freeze 가설 + Mac 절전 (§ 38.8 #1) 재검토. 또는 **F chunk** (ka10001 NUMERIC overflow + sentinel WARN/skipped 분리, 별도 ted-run) |
...
| **30** | **5-14 06:00 OhlcvDaily / 06:30 DailyFlow cron miss** — 재배포 직전 1시간 docker logs 0 cron event | 본 chunk 진단 | **scheduler dead 재발 가능성** — 5-13 17:30/18:00 자연 발화 정상 가설이 5-14 06:00 miss 로 흔들림. 재배포 후 07:00 sector_daily cron 정상 발화 확인. 다음 chunk = dead 재발 분석 (APScheduler timer freeze / Mac 절전 § 38.8 #1) |
...
| 순위 | chunk | 근거 | 예상 규모 |
|------|-------|------|-----------|
| **1** | **scheduler dead 재발 분석 chunk** (5-14 06:00/06:30 cron miss) | § 4 #30. 5-14 새벽 cron 발화 0 — 일회성 가설 흔들림. APScheduler timer freeze 가설 / Mac 절전 (§ 38.8 #1) 재검토. `/admin/scheduler/diag` 발화 직전/후 비교 / 컨테이너 sleep 상태 검증 / cron 정확 시간 timer wakeup 추적 | 진단 + 가설 좁힘 |
