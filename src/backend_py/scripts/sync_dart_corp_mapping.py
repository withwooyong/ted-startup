...
def is_common_stock_code(stock_code: str) -> bool:
    ...
    return len(stock_code) == 6 and stock_code.isdigit() and stock_code.endswith("0")
...
def fetch_krx_listed_codes() -> set[str]:
    ...
    try:
        ...
        print(
            f"[sync] WARN: KRX 상장 리스트 조회 실패 ({exc.__class__.__name__}: {exc})",
            file=sys.stderr,
        )
        print(
...
def filter_by_krx_listing(rows: Iterable[CorpRow], krx_codes: set[str]) -> list[CorpRow]:
