...
def generate_price_series(stock: DemoStock, days: list[date], *, seed: int) -> list[dict[str, object]]:
        ...
        rows.append(
            {
                ...
                "change_rate": Decimal(f"{((close - prev_close) / prev_close) * 100:.4f}")
                if prev_close
                else Decimal("0"),
            }
        )
...
async def run(*, days: int, wipe: bool, seed: int) -> int:
    ...
    print(
        f"[seed-ui] 대상 종목 {len(DEMO_STOCKS)}건 × {days} 영업일 ({trading_days[0]} ~ {trading_days[-1]})",
        flush=True,
    )
    ...
    try:
        async with factory() as session, session.begin():
            ...
            if wipe:
                ...
                existing = (
                    (await session.execute(select(Stock).where(Stock.stock_code.in_(demo_codes)))).scalars().all()
                )
            ...
            accounts = (
                (await session.execute(select(BrokerageAccount).where(BrokerageAccount.is_active.is_(True))))
                .scalars()
                .all()
            )
            ...
            for s in DEMO_STOCKS:
                ...
                price_rows = await price_repo.list_between(sid, trading_days[0], trading_days[-1])
                for pr in price_rows:
                    price_by_date[pr.trading_date][sid] = pr.close_price

            for acc in accounts:
                holdings = (
                    (await session.execute(select(PortfolioHolding).where(PortfolioHolding.account_id == acc.id)))
                    .scalars()
                    .all()
                )
                if not holdings:
                    continue
                total_cost_dec = sum((h.quantity * h.avg_buy_price for h in holdings), start=Decimal("0"))
                for d in trading_days:
                    closes = price_by_date[d]
                    # 보유 중 시드 주가가 없는 종목(=비-데모 종목)은 평단으로 대체
                    total_value_dec = sum(
                        (Decimal(closes.get(h.stock_id, int(h.avg_buy_price))) * h.quantity for h in holdings),
                        start=Decimal("0"),
                    )
